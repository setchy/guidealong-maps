...
const NATIONAL_PARK_SLUGS = new Set([
  "banff-townsite-tour",
  "capitol-reef",
  "delicate-arch",
  ...
  "joshuas-hidden-valley-walking-trail",
  "lake-louise-moraine-yoho",
  "mesa-verde-national-park",
  "redwood-national-and-state-parks-tour",
  "waterton-national-park",
  "yellowstone-national-park",
  "yellowstone-old-faithful-geyser-basin-walk",
  "yellowstone-west-thumb-geyser-basin-walk",
  "yellowstones-grand-prismatic-fairy-falls-trail",
...
function deriveCategory(title, url) {
  if (/\bnational park\b/i.test(title) || NATIONAL_PARK_SLUGS.has(slugOf(url)))
...
function parseTourItem($, link) {
  ...
  if (listImg?.length) {
    const raw = listImg.attr("data-lazy-src") || listImg.attr("data-src") || "";
  ...
  return {
    title,
    url,
    category: deriveCategory(title, url),
    details: {
      description,
      thumbnail,
      audioPoints,
      ...
      for (const item of items) {
        if (item.url && !seenUrls.has(item.url)) {
          ...
          tours.push({
            title: item.title,
            url: item.url,
            category: deriveCategory(item.title, item.url),
            details: {
...
module.exports = { deriveCategory, discoverInventory };
